...
export function SearchEmpty({ query, onSuggestionClick }: SearchEmptyProps) {
  ...
  return (
    <div className="search-empty animate-fade-in">
      <InboxIcon className="search-empty-icon size-10 text-muted-foreground opacity-60" />
      <h3 className="search-empty-text text-sm">
        {query ? `没有找到关于 “${query}” 的相关结果` : '搜索文章'}
      </h3>
      <p className="search-empty-subtext mt-1 text-xs text-muted-foreground">
